...
summStats <- mclapply(1:length(obsdat), mc.cores = nthrd, FUN = function(i) {
    ...
    o <- sapply(1:ncol(x), function(j) {
        ...
        if(class(params) == 'try-error') {
            params <- c(size = NA, mu = NA, loglik = NA, poisLogLik = NA)
        } else {
            poisLogLik <- sum(dpois(x[, j], params$estimate[2], log = TRUE))
            params <- c(params$estimate, loglik = params$loglik, 
                        poisLogLik = poisLogLik)
        }
...
simNBPerm <- ssadSim(30, 50, nthrd, function(n) {rfish(n, 0.01)},
                     function(n, mu) {rnbinom(n, 0.05, mu = mu)}, nsim = 500, B = 999)
