...
fig2bc <- function(x, breaksRho, breaksWM) {
...
```{r ssad_plot, fig.width = 5, fig.height = 5, cache = FALSE}
...
makeData <- function(nsite, nspp, ssadfun = 'nbinom') {
    # make SAD
    ...
    if(ssad == 'nbinom') {
        ...
        k <- exp(predict(kByRelSppMod, newdata = data.frame(loga = log(abund / J),
                                                            logS = log(nspp))) + 
                     rnorm(nspp, sd = summary(kByRelSppMod)$sigma))
        o <- sapply(1:length(abund), function(i) {
            return(rnbinom(nsite, k[i], mu = mu[i]))
        })
    } else {
        o <- sapply(1:length(abund), function(i) {
            return(rpois(nsite, mu[i]))
        })
    }
...
simPlusMinus <- mclapply(1:nsim,  mc.cores = nthrd, 
                         FUN = function(i) {
                             ...
                             if(any(dim(simDat) < 10)) {
                                 o <- rep(NA, ncol(commStats))
                                 names(o) <- names(commStats)
                             } else {
                                 o <- unlist(plusMinus(simDat))
                             }
...
simPlusMinus <- simPlusMinus[!is.na(simPlusMinus$pos.n), ]
```

```{r sim_plusMinus_plot, fig.width = 7.5, fig.height = 4, cache = FALSE}
wmmax <- ceiling(max(simPlusMinus$pos.wm, simPlusMinus$neg.wm, na.rm = TRUE) * 9) / 3

fig2bc(simPlusMinus, breaksRho = seq(-2, 2, by = 1/5), 
...
# loop over simulation replicates
simPois <- mclapply(1:nsim,  mc.cores = nthrd, 
                         FUN = function(i) {
                             j <- sample(nrow(nsitenspp), 1)
                             simDat <- makeData(nsite = nsitenspp$nsite[j], 
                                                nspp = nsitenspp$nspp[j])
                             ...
                             if(any(dim(simDat) < 10)) {
                                 ...
                                 o <- unlist(plusMinus(simDat, ssadfun = 'pois'))
                             }
...
simPois <- as.data.frame(do.call(rbind, simPois))
simPois <- simPois[!is.na(simPois$pos.n), ]
```

```{r sim_pois_plusMinus_plot, fig.width = 7.5, fig.height = 4, cache = FALSE}
wmmax <- ceiling(max(simPois$pos.wm, simPois$neg.wm, na.rm = TRUE) * 9) / 3

fig2bc(simPois, breaksRho = seq(-2, 2, by = 1/5), 
       breaksWM = seq(-wmmax, wmmax, by = 0.1/3))
```
