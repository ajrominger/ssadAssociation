---
title: "Supplement to: On the inference of positive and negative interactions and their relation to abundance"
...
```{r keeplatex, include = FALSE, cache = FALSE, echo = FALSE}
options(tinytex.clean = FALSE)
```

This supplement combines a narrative account of how to reproduce my results as well as additional analyses in support of the conclusions in the main text.

# Reproducibility

The results of this study can be fully reproduced by installing the package *RarePlusComMinus* (availible on GitHub) writen in R [@rcore].  Installation requires the package *devtools* [@devtools]:
...
Two additional custom packages are required, *socorro* [@socorro] for plotting, and *pika* [@pika] for simulating and analyzing species abundance distributions:

```{r customPacks, results = 'hide', warning = FALSE, message = FALSE}
...
```{r, helpdocs, eval = FALSE, cache = FALSE}
?plusMinus
?schoener
```
...
```{r pkg_setup, cache = FALSE, results = 'hide', warning = FALSE, message = FALSE}
...
Now we use the `plusMinus` function to calculate positive and negative association networks and abundances from the observed data:
...
I use these calculations to make Supplementary Figure \ref{fig:plusMinus_obs}, which reproduce Figure 2 (b-c) from CEA [@calatayud2019].
...
It should be noted that my inset plot of mean weighted abundances (Supplementary Fig. \ref{fig:plusMinus_obs} B) differs from the original paper [@calatayud2019] in that their y-axis ranges from 0 to 220, while mine ranges from 0 to 120; I suspected the original authors mislabled their axis, because the scale as presented would suggest over `r 100 * floor(fig2est / 100)` assembledges, while the number should be 326.  Rescaling their axis to range from 0 to 120 brings the rough estiamte from their figure more in line with the reported number of assembledges.

# Exploring patterns of species abundance across space

Now we explore the shape of the spatial abundance patters in the real data provided by CEA [@calatayud2019].  The main function we use is `nbFit` from the *RarePlusComMinus* package which calculates summary statistics about the negative binomial and Poisson fits to spatial abundance data.

In Supplementary Figure \ref{fig:ssad} I show that the data are well fit by the negative binomial via a likelihood-based goodness of fit test [@etienne2007; @romMeteR]. This test scales the observed likelihood by the sampling distribution of likelihoods given the hypothesis that the negative binomial is the correct distribution. This test stastistic, when squared, follows a $\chi$-squared distribution with 1 degree of freedom [@romMeteR], allowing us to make a parametric cuttoff of when the data are not well represented by a negative binomial.  No assembledge analyzed rejected the negative binomial distribution. In Supplementary Figure \ref{fig:ssad} I also explore the relationship of the clustering parameter $k$ with species relative abundance.  The purpose of this later analysis is to be able to simulate random but realistic data. 
...
We also need to know the shapes of the species abundance distributions (SAD) of each assembledge to simulate realistic data. I do this using the function `fitSAD` from the custom *pika* package. I preform model selection on three standard SAD forms: the log-series, Poisson log-normal, and zero-truncated negative binomial, and record the best fit model and its parameter(s) for each assembledge.
...
Now we can simulate abundance data matching the overall shapes of the observed SAD's and SSAD's but with absolutely no real correlation or interaction between species.

```{r sim_plusMinus_setup}
# number of simulations to run
nsim <- round(1.25 * sum(!is.na(commStats$pos.wm)))
```

I simulate `r nsim` random assembledges, slightly more than the `r sum(!is.na(commStats$pos.wm))` observed assemledges because some simulated assembledges will be rejected based on the data standards used. In Figure 1 of the main text we plot these simulated results alongside the results from the real data.
...
```{r sim_plusMinus_plot, fig.width = figW * 1.95, fig.height = 4, eval = FALSE, cache = FALSE, include = FALSE}
...
```{r sim_pois_plusMinus_plot, fig.width = figW * 1.95, fig.height = 4, eval = FALSE, cache = FALSE, include = FALSE}
...
```{r, sim_simple_nb_plusMinus_setup}
oneK <- 0.1
b <- 0.01
nsiteSimp <- 20
nsppSimp <- 50
...
In this case I use a log-series SAD with $\beta =$ `r b`, a negative binomial with $k = $`r oneK`, and consider an assembledge with on average `r nsiteSimp` sites and `r nsppSimp` species.

```{r, sim_simple_nb_plusMinus_comp, dependson = 'sim_simple_nb_plusMinus_setup'}
oneK <- 0.1
b <- 0.01
nsiteSimp <- 20
nsppSimp <- 50
...
```{r sim_simple_plot, fig.width = figW * 1.95, fig.height = figH * 1.95, cache = FALSE, fig.cap = "bla bla bla \\label{fig:simpleSim}"}
split.screen(c(2, 1))

screen(1)
fig2bc(simPMSimpNB, breaksRho = seq(-2, 2, by = 1/5), 
       breaksWM = seq(-0.3, 0.3, by = 0.1/3), addxlab = FALSE, figLabs = c('A', 'B'))

screen(2)
fig2bc(simPMSimpPo, breaksRho = seq(-2, 2, by = 1/5), 
       breaksWM = seq(-0.3, 0.3, by = 0.1/3), figLabs = c('C', 'D'))

close.screen(all.screens = TRUE)
```

To understand why these sperious results occur we must understand what the fixed-fixed null model [@ulrich2010; @patefield1981] does to the underlying SSAD.  We know that the fixed-fixed algorithm preserves, by definition, the SAD and the total abundances across sites, but within any given species, the allocation of its abundances has a potentially large combinatorial space.  In Supplementary Figure \ref{fig:ssadPerm} I show that the fixed-fixed permutation algorithm has no net effect on Poisson SSAD's, but when the original SSAD's come from a negative binomial, the fixed-fixed algorithm tends to spread out abundances acorss more sites, and consiquently inflates the inferred $k$ parameters of those SSAD's. This has the overall effect of pushing observed negative binomial SSAD's more toward a Poisson shape.
...
simNBPerm <- ssadSim(nsite, nspp, nthrd, function(n) {rfish(n, 0.01)},
                     function(n, mu) {rnbinom(n, 0.1, mu = mu)}, nsim = nsim)

simPoPerm <- ssadSim(nsite, nspp, nthrd, function(n) {rfish(n, 0.01)},
                     function(n, mu) {rpois(n, lambda = mu)}, nsim = nsim)
```


```{r ssad_perm_plot, cache = FALSE, fig.width = figW * 1.95, eval = FALSE}
...
Finally we very simply want to understand at a mathematical level why a spatially clustered verusus spatially even SSAD would lead to these results.  To explore this we consider a very simple example of two species and their Sch similarity.  
...
First we consider two rare species, one with abundance 1, and the other with abundance `r Nrare`.  If we consider dataset with `r nsite` total sites, then the configuration that maximizes the Schoener similarity between these two species is
$$
`r mathmat(rarePair)`
$$
We can calcuate how likely this configuration is under a negative binomial versus a Poisson SSAD like this
...
```{r mathEG_comm, dependson = 'mathEG_setup'}
...
```{r mathEG_commMax, dependson = 'mathEG_setup'}
nbCommPMax <- prod(dnbinom(as.vector(commPairMax), k, mu = Ncomm / nsite))
poCommPMax <- prod(dpois(as.vector(commPairMax), Ncomm / nsite))
```

The conclusions of these probability calculations are disscussed in the main text. 
